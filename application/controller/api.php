...
class api extends Controller
{
    ...
    $new_sql="UPDATE ftd  SET user_id='17', ftd_date='2018-06-13 17:25:58 15:51:31',client_id='2833230',client_name='  GIANNI SARDELLA',client_email='gianni.ele44@gmail.com',amount='230',retention_id='',note='Fatto 230 euro nel nome de la moglie utilizza una paypal visa prepagata datto il credito di 100 lui e sposato lavora come mecanico non ha mai fatto trading ha solo sentito fatto vedere il petrolio greggio parlato per le notizielui vuole operare gia da oggi si lamenta ma i soldi non mancanoChiamare verso le 17 e 30 da un adetto per scaricare la piataforma',currency='EUR'  WHERE ftd_id ='487' ";
    $old_sql="UPDATE ftd  SET user_id='11', ftd_date='2018-06-13 17:25:58 15:51:31',client_id='2833230',client_name='  GIANNI SARDELLA',client_email='gianni.ele44@gmail.com',amount='230',retention_id='',note='Fatto 230 euro nel nome de la moglie utilizza una paypal visa prepagata datto il credito di 100 lui e sposato lavora come mecanico non ha mai fatto trading ha solo sentito fatto vedere il petrolio greggio parlato per le notizielui vuole operare gia da oggi si lamenta ma i soldi non mancanoChiamare verso le 17 e 30 da un adetto per scaricare la piataforma',currency='EURk'  WHERE ftd_id ='487' ";
    $new_sql = preg_replace('/\',/i', '\' ,',$new_sql);
    $new_sql = preg_replace('/ ,/', "\n",$new_sql);
    $old_sql = preg_replace('/\',/i', '\' ,',$old_sql);
    $old_sql = preg_replace('/ ,/', "\n",$old_sql);

    // output the result of comparing two files as HTML
    echo Diff::toTable(Diff::compare($old_sql, $new_sql));
    //echo Diff::toTable(Diff::compare("test", "test1"));
    }
}
